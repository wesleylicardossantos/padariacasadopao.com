...
<div class="page-content">
    <div class="func-tabbar">
        <a href="<?php echo e(route('funcionarios.index', ['status' => 1])); ?>" class="func-tab <?php echo e(!$arquivoMorto ? 'active' : ''); ?>">
            <i class="bx bx-user-check"></i>
            <span>Funcionários ativos</span>
            <span class="badge bg-success"><?php echo e($resumoLista['ativos'] ?? 0); ?></span>
        </a>
        <a href="<?php echo e(route('funcionarios.index', ['arquivo_morto' => 1, 'status' => 0])); ?>" class="func-tab <?php echo e($arquivoMorto ? 'active' : ''); ?>">
            <i class="bx bx-archive-in"></i>
            <span>Arquivo morto</span>
            <span class="badge bg-secondary"><?php echo e($resumoLista['inativos'] ?? 0); ?></span>
        </a>
    </div>
    ...
    <div class="card func-card">
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3">
                <div>
                    <h5 class="mb-0"><?php echo e($arquivoMorto ? 'Arquivo morto de funcionários' : 'Funcionários ativos'); ?></h5>
                    <small class="text-muted">
                        <?php echo e($arquivoMorto ? 'Colaboradores inativos arquivados para consulta e histórico.' : 'Visão ERP integrada ao RH com foco apenas nos colaboradores ativos.'); ?>

                    </small>
                </div>
                <div class="d-flex gap-2 flex-wrap">
                    <a href="<?php echo e(route('funcionarios.exportPdf', $filters)); ?>" target="_blank" class="btn btn-danger"><i class="bx bx-file"></i> PDF</a>
                    <a href="<?php echo e(route('funcionarios.exportExcel', $filters)); ?>" class="btn btn-dark"><i class="bx bx-spreadsheet"></i> Excel</a>
                    <a href="/rh" class="btn btn-primary"><i class="bx bx-bar-chart-alt"></i> Dashboard RH Executivo</a>
                    <?php if (! ($arquivoMorto)): ?>
                    <a href="<?php echo e(route('funcionarios.create')); ?>" class="btn btn-success"><i class="bx bx-plus"></i> Novo funcionário</a>
                    <?php endif; ?>
                </div>
            </div>

            <div class="d-flex gap-3 flex-wrap mb-3">
                <div class="func-kpi"><span class="text-muted">Total cadastrado</span><strong><?php echo e($resumoLista['total'] ?? 0); ?></strong></div>
                <div class="func-kpi"><span class="text-muted">Ativos</span><strong><?php echo e($resumoLista['ativos'] ?? 0); ?></strong></div>
                <div class="func-kpi"><span class="text-muted">Inativos arquivados</span><strong><?php echo e($resumoLista['inativos'] ?? 0); ?></strong></div>
            </div>

            <?php echo Form::open()->fill($filters)->get(['id' => 'funcionarios-filter-form']); ?>

            <?php echo Form::hidden('arquivo_morto', $arquivoMorto ? 1 : 0); ?>

            <div class="row g-3 align-items-end">
                <div class="col-lg-4 col-md-6"><?php echo Form::text('nome', 'Pesquisar por nome')->attrs(['id' => 'filtro-nome', 'autocomplete' => 'off']); ?></div>
                <div class="col-lg-4 col-md-6"><?php echo Form::text('cpf', 'Pesquisar por CPF')->attrs(['id' => 'filtro-cpf', 'maxlength' => '14', 'autocomplete' => 'off'])->type('tel'); ?></div>
                <div class="col-lg-3 col-md-6"><?php echo Form::select('status', 'Status')->options(['' => 'Todos', '1' => 'Ativos', '0' => 'Inativos'])->attrs(['id' => 'filtro-status']); ?></div>
                <div class="col-lg-1 col-md-6">
                    <div class="d-flex gap-2">
                        <button class="btn btn-primary w-100" type="submit"><i class="bx bx-search"></i></button>
                        <a class="btn btn-danger" href="<?php echo e(route('funcionarios.index', $arquivoMorto ? ['arquivo_morto' => 1, 'status' => 0] : ['status' => 1])); ?>"><i class="bx bx-eraser"></i></a>
                    </div>
                </div>
            </div>
            <?php echo Form::close(); ?>
    ...
    <div class="card func-card mt-3">
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-striped func-table mb-0" id="funcionarios-table">
                    ...
                    <tbody id="funcionarios-table-body">
                        ...
                        ?>
                        <tr data-status="<?php echo e($statusAtivo ? '1' : '0'); ?>">
                            <td><?php echo e(method_exists($data, 'firstItem') && $data->firstItem() ? $data->firstItem() + $loop->index : $loop->iteration); ?></td>
                            <td class="func-cpf"><?php echo e($item->cpf); ?></td>
                            <td><?php echo e($item->nome); ?></td>
                            <td><?php echo e($dataAdmissao); ?></td>
                            <td><?php echo e($item->funcao ?? '-'); ?></td>
                            <td><?php echo e(function_exists('__moeda') ? __moeda($item->salario) : number_format((float)$item->salario, 2, ',', '.')); ?></td>
                            <td>
                                <?php if($statusAtivo): ?>
                            ...
                            <td class="text-nowrap">
                                <div class="func-actions">
                                    ...
                                    <div class="func-menu">
                                        <a href="<?php echo e(route('funcionarios.imprimir', $item->id)); ?>" target="_blank"><i class="bx bx-printer"></i> Imprimir ficha</a>
                                        <a href="<?php echo e(route('funcionarios.show', $item->id)); ?>"><i class="bx bx-detail"></i> Ver detalhes</a>
                                        <a href="<?php echo e(route('funcionarios.edit', $item->id)); ?>"><i class="bx bx-edit"></i> Editar cadastro</a>
                                        <a href="<?php echo e(route('rh.dossie.show', $item->id)); ?>"><i class="bx bx-folder-open"></i> Dossiê RH</a>
                                        <a href="<?php echo e(route('funcionarios.toggleStatus', $item->id)); ?>" onclick="return confirm('Deseja realmente <?php echo e($statusAtivo ? 'arquivar' : 'reativar'); ?> este funcionário?')"><i class="bx <?php echo e($statusAtivo ? 'bx-archive-in' : 'bx-user-check'); ?>"></i> <?php echo e($statusAtivo ? 'Mover para arquivo morto' : 'Reativar funcionário'); ?></a>
                                        <form action="<?php echo e(route('funcionarios.destroy', $item->id)); ?>" method="POST" id="funcionario-delete-<?php echo e($item->id); ?>" class="m-0">
                                            <?php echo csrf_field(); ?>
                        ...
                        <tr><td colspan="8" class="text-center py-4 text-muted"><?php echo e($arquivoMorto ? 'Nenhum funcionário inativo arquivado.' : 'Nenhum funcionário ativo encontrado.'); ?></td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 px-3 py-3 border-top">
                <small class="text-muted">
                    <?php if(method_exists($data, 'firstItem') && $data->total()): ?>
                        Exibindo <?php echo e($data->firstItem()); ?> a <?php echo e($data->lastItem()); ?> de <?php echo e($data->total()); ?> registros
                    <?php else: ?>
                        Total de registros: <?php echo e(count($data)); ?>

                    <?php endif; ?>
                </small>
                <?php echo $data->appends($filters)->links(); ?>
...
<?php echo $__env->make('default.layout',['title' => 'Funcionários'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home2/wesl4494/public_html/padariacasadopao.com/resources/views/funcionarios/index.blade.php ENDPATH**/ ?>