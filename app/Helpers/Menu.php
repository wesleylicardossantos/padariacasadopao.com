...
[
	...
	'subs' => [
		...
		[
			...
			'rota' => '/rh/funcionario-eventos'
		],
		[
			'nome' => 'Apuração Mensal',
			'rota' => '/rh/apuracao-mensal'
		],
		[
			'nome' => 'Dashboard SaaS Executivo',
			'icone' => 'bi bi-bar-chart-line-fill',
			'rota' => route('enterprise.saas.executive')
		],
		[
			'nome' => 'RBAC RH Profissional',
			'icone' => 'bi bi-shield-lock-fill',
			'rota' => route('rh.acl.index')
		],
		[
			'nome' => 'SaaS Scale Ops',
			'icone' => 'bi bi-cpu-fill',
			'rota' => route('enterprise.saas.scale')
		],
		[
			'nome' => 'Observability Center',
			'icone' => 'bi bi-activity',
			'rota' => route('enterprise.saas.observability')
		],
		[
			'nome' => 'Dashboard RH Executivo',
			'rota' => '/rh',
			'exact_match' => true
		],
		[
		...
		[
			'nome' => 'Desligamentos',
			'rota' => '/rh/desligamentos'
		],
		[
			'nome' => 'Cálculo de Férias',
			'rota' => '/rh/ferias/calculo'
		],
		[
			'nome' => 'DRE Inteligente',
			'rota' => '/rh/dre-inteligente'
		],
		[
			'nome' => 'DRE Preditivo',
			'rota' => '/rh/dre-preditivo'
		],
		[
			'nome' => 'IA de Decisão',
			'rota' => '/rh/ia-decisao'
		...
		[
			'nome' => 'Resumo Financeiro RH',
			'rota' => '/rh/folha/resumo-financeiro'
		],
		[
			'nome' => 'Documentos Inteligentes',
			'rota' => '/rh/documentos'
		],
		[
			'nome' => 'Templates Jurídicos RH',
			'rota' => '/rh/documentos/templates'
		],
	]
],
			[
				'titulo' => 'Controle',
				'icone' => 'bx bx-slider-alt',
				'subs' => [
					[
						'nome' => 'Controle de Pães',
						'rota' => route('controle.paes')
					],
					[
						'nome' => 'Controle de Salgados',
						'rota' => route('controle.salgados.index')
					]
				]
			],
			...
			[
				...
				'subs' => [
					[
						'nome' => 'Painel de Precificação',
						'rota' => route('precificacao.index')
					],
					[
						'nome' => 'Insumos',
						'rota' => route('precificacao.insumos.index')
					],
					[
						'nome' => 'Fichas Técnicas',
						'rota' => route('precificacao.receitas.index')
					],
					[
						'nome' => 'Mapeamentos',
						'rota' => route('precificacao.mapeamentos.index')
					],
					[
						'nome' => 'Sugestões de Preço',
						'rota' => route('precificacao.sugestoes.index')
					],
					[
						'nome' => 'Dashboard Executivo',
						'rota' => route('precificacao.dashboard-executivo.index')
					],
					[
						'nome' => 'Canais de Venda',
						'rota' => route('precificacao.canais.index')
					],
					[
						'nome' => 'Lista de Compras',
						'rota' => route('precificacao.lista-compras.index')
					],
					[
						'nome' => 'Produção',
						'rota' => route('precificacao.producao.index')
					]
				]
			],
			[
			...
			[
				...
				'subs' => [
					...
					[
						'nome' => 'Relatório de Vendas - Resumido',
						'rota' => route('controle.relatorio-vendas-resumido')
					],
					[
						'nome' => 'Orçamentos',
	...
	{
		return $this->filterByPermissions($this->menu);
	}

	private function filterByPermissions(array $items): array
	{
		$acl = app(RHAccessControlService::class);

		return array_values(array_filter(array_map(function ($item) use ($acl) {
			if (isset($item['permissao']) && !$acl->userCan((string) $item['permissao'])) {
				return null;
			}

			if (isset($item['subs']) && is_array($item['subs'])) {
				$item['subs'] = $this->filterByPermissions($item['subs']);
				if (empty($item['subs'])) {
					return null;
				}
			}
