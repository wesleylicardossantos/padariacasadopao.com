...
ini_set('display_errors', app()->environment('local') ? 'On' : 'Off');
...
class NFService{
	...
	public function __construct($config, $emitente){
        ...
        $this->enableLegacyOpenSSLIfAvailable();
        $this->validateCertificateBeforeSend($emitente);

        $certificate = $this->loadCertificateSafely($emitente);
...
private function enableLegacyOpenSSLIfAvailable(): void
{
        try{
                $conf = base_path('openssl_legacy.cnf');
                if(function_exists('putenv') && is_file($conf) && is_readable($conf)){
                        putenv('OPENSSL_CONF=' . $conf);
                        $_ENV['OPENSSL_CONF'] = $conf;
                        $_SERVER['OPENSSL_CONF'] = $conf;
                }
        }catch(\Throwable $e){
                Log::warning('Falha ao configurar OPENSSL legacy (NFService)', [
...
private function validateCertificateBeforeSend($emitente): void
{
        $certPath = $this->resolveCertificatePath($emitente->arquivo);

        if(!is_file($certPath)){
                throw new \RuntimeException('Arquivo do certificado não encontrado: ' . $certPath);
        }

        if(!is_readable($certPath)){
                throw new \RuntimeException('Arquivo do certificado sem permissão de leitura: ' . $certPath);
        }

        if(empty($emitente->senha)){
                throw new \RuntimeException('Senha do certificado não informada.');
        }
}

private function loadCertificateSafely($emitente): Certificate
{
        $certPath = $this->resolveCertificatePath($emitente->arquivo);
        $content = @file_get_contents($certPath);

        if($content === false || $content === ''){
                throw new \RuntimeException('Não foi possível ler o conteúdo do certificado: ' . $certPath);
        }

        try{
                return $this->readCertificateFromContent($content, $emitente->senha);
        }catch(\Throwable $e){
                Log::error('Falha ao ler certificado A1 (NFService)', [
                        'empresa_id' => $this->empresa_id,
                        'arquivo' => $emitente->arquivo,
                        'cert_path' => $certPath,
                        'erro' => $e->getMessage(),
                ]);

                throw new \RuntimeException(
                        'Impossível ler o certificado digital. Verifique senha, formato do PFX e compatibilidade OpenSSL.'
                );
        }
}

private function resolveCertificatePath(string $arquivo): string
{
        if(str_starts_with($arquivo, '/') && is_file($arquivo)){
                return $arquivo;
        }

        $candidates = [
                $arquivo,
                storage_path('app/' . ltrim($arquivo, '/')),
                storage_path(ltrim($arquivo, '/')),
                public_path(ltrim($arquivo, '/')),
                base_path(ltrim($arquivo, '/')),
        ];
