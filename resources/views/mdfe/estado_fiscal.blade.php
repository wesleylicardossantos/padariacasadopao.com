@extends('default.layout',['title' => 'Alterar Status Mdfe'])
@section('content')
@section('css')
<style type="text/css">
    input[type="file"] {
    ...
    <div class="card ">
        <div class="card-body p-4">
            ...
            <div class="page-breadcrumb">
                <div class="row m-3">
                    <h3>MDFe código: <strong class="text-info">{{ $item->id }}</strong></h3>
                    <hr>
                    {{-- <h5> Natureza de operação: <strong class="text-info"></strong></h5> --}}
                    <h5> Data de registro: <strong class="text-info">{{ __data_pt($item->created_at, 0) }}</strong></h5>
                    <h5> Valor de carga: <strong class="text-info">R$ {{ __moeda($item->valor_carga) }}</strong></h5>
                    {{-- <h5> Valor a receber: <strong class="text-info">R$ </strong></h5> --}}
                </div>
            </div>
            {!!Form::open()
            ->post()
            ->route('mdfe.estadoFiscalStore')
            ->multipart()
            !!}
            <input type="hidden" name="mdfe_id" value="{{$item->id}}">
            <div class="col-3 m-4">
