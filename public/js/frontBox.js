...
function apontarComanda(){
    ...
    try{
        msg = err.responseJSON?.message || err.responseJSON || msg
    }catch(e){}
...
function validateButtonSave() {
    ...
    var valor_recebido = convertMoedaToFloat($('#valor_recebido').val())
    ...
    setTimeout(() => {
        ...
        if (barcode.length > 7) {
            ...
            .done((e) => {
                ...
                if (e.valor_venda) {
                    $('#inp-produto_id').html('')
                    var newOption = new Option(e.nome, e.id, 1, false);

                    $('#inp-produto_id').append(newOption).change();
                    $("#inp-quantidade").val("1,00");
...
function buscarPorReferencia(barcode) {
    ...
    .done((e) => {
        ...
        $("#inp-quantidade").val(convertFloatToMoeda(e.quantidade));
